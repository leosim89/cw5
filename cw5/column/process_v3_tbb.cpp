...
{
	if (!input.empty()){
		if (left.empty()) {	//First column
			output[0] = vmin(input[0], input[1], right[0]);
			for (unsigned x = 1; x < h - 1; x++){
				output[x] = vmin(input[x], input[x + 1], input[x - 1], right[x]);
			}
			output[h - 1] = vmin(input[h - 1], input[h - 2], right[h - 1]);
		}
		else if (right.empty()) {
			output[0] = vmin(input[0], input[1], left[0]);
			for (unsigned x = 1; x < h - 1; x++){
				output[x] = vmin(input[x], input[x + 1], input[x - 1], left[x]);
			}
			output[h - 1] = vmin(input[h - 1], input[h - 2], left[h - 1]);
		}
		else {
			output[0] = vmin(input[0], input[1], left[0], right[0]);
			for (unsigned x = 1; x < h - 1; x++){
				output[x] = vmin(input[x], input[x + 1], input[x - 1], left[x], right[x]);
			}
			output[h - 1] = vmin(input[h - 1], input[h - 2], left[h - 1], right[h - 1]);
		}
	}
}
	...
	std::vector<uint32_t> &output)
{
	if (!input.empty()){
		if (left.empty()) {	//First column
			output[0] = vmax(input[0], input[1], right[0]);
			for (unsigned x = 1; x < h - 1; x++){
				output[x] = vmax(input[x], input[x + 1], input[x - 1], right[x]);
			}
			output[h - 1] = vmax(input[h - 1], input[h - 2], right[h - 1]);
		}
		else if (right.empty()) {
			output[0] = vmax(input[0], input[1], left[0]);
			for (unsigned x = 1; x < h - 1; x++){
				output[x] = vmax(input[x], input[x + 1], input[x - 1], left[x]);
			}
			output[h - 1] = vmax(input[h - 1], input[h - 2], left[h - 1]);
		}
		else {
			output[0] = vmax(input[0], input[1], left[0], right[0]);
			for (unsigned x = 1; x < h - 1; x++){
				output[x] = vmax(input[x], input[x + 1], input[x - 1], left[x], right[x]);
			}
			output[h - 1] = vmax(input[h - 1], input[h - 2], left[h - 1], right[h - 1]);
		}
	}
}
...
int main(int argc, char *argv[])
{
	try{
		...
		auto fwd = levels < 0 ? erode_column : dilate_column;
		auto rev = levels < 0 ? dilate_column : erode_column;
		...
		while (1) {
			...
			for (unsigned i = 1; i < (w + 4*std::abs(levels)-1); i++){ 
				...
				auto process_buffers = [=, &left, &right, &center, &output](unsigned l){
				...
				tbb::parallel_for(0, 2 * std::abs(levels), process_buffers);

				// Take in new sample chunk
				if (i < w){
					t[i] = clock();
					if (!read_blob(STDIN_FILENO, cbRaw, &raw[0]))
						break;
				...
				// Generate Results
				auto result_gen = [=, &output](unsigned l){
					//auto op = [&](unsigned x){ return output[l]; };
					fwd(h, center[l], right[l], left[l], output[l]);
					rev(h, center[l + std::abs(levels)], right[l + std::abs(levels)], left[l + std::abs(levels)], output[l + std::abs(levels)]);
				};
				tbb::parallel_for(0, std::abs(levels), result_gen);
